...
export const user = pgTable(
	...
	(table) => [unique("user_jwt_sub_key").on(table.jwtSub)],
);
...
export const asset = pgTable(
	...
	(table) => [
		...
		check(
			"asset_type_check",
			sql`(asset_type)::text = ANY ((ARRAY['bgm'::character varying, 'soundEffect'::character varying, 'characterImage'::character varying, 'backgroundImage'::character varying, 'cgImage'::character varying])::text[])`,
		),
	],
);
...
export const gameCategory = pgTable(
	...
	(table) => [unique("game_category_name_key").on(table.name)],
);
