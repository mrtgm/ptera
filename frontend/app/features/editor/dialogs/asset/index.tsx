...
export const AssetDialog = ({
	...
	game: Game;
	onConfirmAssetSelection: (assetId: string, type: AssetType) => void;
	onDeleteAsset: (assetId: string, type: AssetType) => void;
}) => {
	const modalSlice = useStore.useSlice.modal();
	const navigate = useNavigate();

	const [activeTab, setActiveTab] = useState<AssetType>("backgroundImages");
	...
	useEffect(() => {
		...
		if (
			...
			setActiveTab(
				(modalSlice.params as ModalParams["asset"])?.target as AssetType,
			);
		}
	...
	// アセットの種類によってプレビュータイプを決定
	const getPreviewType = (type: AssetType) => {
		switch (type) {
